/**
 * @title OwnershipTransfers Tests
 * @notice Tests two-step ownership transfer flows across identity contracts
...
describe("Ownership transfers", () => {
  ...
  async function deployCompliantERC20(checkerAddress: string) {
    ...
    const contract = await factory.deploy("Compliant Token", "CPL", checkerAddress);
    await contract.waitForDeployment();
    return contract;
  }
